 ...
 * NOTE: multiple delimiters are not handled, th9s routine takes the first only
 * @param text
 * @param delimStart (can be a string)
 * @param delimStop (can be a string)
 * @return
 */
...
unsigned last = str.find(delimStop,first+1);
if (first != string::npos && last != string::npos)
	return str.substr(first+delimStart.length(), last - first - delimStart.length());
else
	return "";
}
