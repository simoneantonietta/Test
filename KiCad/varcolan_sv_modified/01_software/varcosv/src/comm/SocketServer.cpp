...
if ((masterSocket = socket(AF_INET, SOCK_STREAM, 0)) == 0)
	...
	dbg->trace(DBG_ERROR, "socket failed");
	return false;
	}
...
if (setsockopt(masterSocket, SOL_SOCKET, SO_REUSEADDR, (char *) &opt, sizeof(opt))
    < 0)
	{
	dbg->trace(DBG_ERROR, "setting socket options");
	return false;
	}
...
if (bind(masterSocket, (struct sockaddr *) &address, sizeof(address)) < 0)
	{
	dbg->trace(DBG_ERROR, "bind failed");
	return false;
	}
dbg->trace(DBG_NOTIFY, "Listener on port %d", this->port);

//try to specify maximum of 3 pending connections for the master socket
if (listen(masterSocket, SKSRV_MAX_PENDING_CONN) < 0)
	{
	dbg->trace(DBG_ERROR, "on listener");
	return false;
	}

//accept the incoming connection
this->addrlen = sizeof(address);
dbg->trace(DBG_NOTIFY, "Waiting for connections...");
return true;
}
...
if ((activity < 0) && (errno != EINTR))
	{
	dbg->trace(DBG_ERROR, "select error");
	}
...
if (FD_ISSET(masterSocket, &readfds))
	...
	dbg->trace(DBG_NOTIFY, "New connection, socket fd is %d, ip is : %s, port : %d", newSocket, inet_ntoa(address.sin_addr), port);
	...
	for (int i = 0; i < SKSRV_MAX_CLIENTS; i++)
		...
		if (clData[i].clientSocket == 0)
			...
			dbg->trace(DBG_NOTIFY, "Adding to list of sockets as client  %d", i);
			onNewConnection(i);
			break;
			}
		}
	if (!found) dbg->trace(DBG_ERROR, "too many client attached");
	}
	...
	for (int i = 0; i < SKSRV_MAX_CLIENTS; i++)
		...
		if (FD_ISSET(sd, &readfds))
			...
			if ((valread = read(sd, clData[i].buffer, SKSRV_BUFFER_SIZE)) == 0)
				...
				dbg->trace(DBG_NOTIFY, "Host disconnected, client %d, ip %s , port %d", i, inet_ntoa(address.sin_addr), port);
				onCloseConnection(i);
...
if (nbytes != size)
	{
	dbg->trace(DBG_ERROR, "not all data are sent (%d/%d)",nbytes,size);
	return false;
	}
#ifdef DEBUG_SKT
...
if (clData[client].nDataAvailable > 0)
	...
	if (size < clData[client].nDataAvailable)
	  dbg->trace(DBG_WARNING, "destination data buffer smaller than the available data");
	memcpy(data, clData[client].buffer, size);
	clData[client].nDataAvailable = 0;
	return true;
	}
dbg->trace(DBG_WARNING, "attempt to read with no data available");
return false;
}
...
if (textMode)
	...
	dbg->trace(DBG_DEBUG, "RX: client %d (%d bytes)\n--------------------------------------------------------------------------\n%s--------------------------------------------------------------------------",client,size,str.c_str());
	}
}
...
if (textMode)
	...
	string str="";
	hexdumpStr(str,buff,size);
	dbg->trace(DBG_DEBUG, "TX: client %d (%d bytes)\n--------------------------------------------------------------------------\n%s--------------------------------------------------------------------------",client,size,str.c_str());
	}
}
