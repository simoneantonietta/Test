...
while(!endApplicationStatus())
	...
	onDataRaw(pdrx, rxBuffer, rxndata);
	do
		{
		initial_ndata = rxndata;	// store temporary (debug usage)
		rxndata = hprotFrameParserNData(pdrx, &f, rxBuffer, rxndata);
		if(isPacketProtocol())
			{
		...
		//------------------
		if(pdrx->protCondition == pc_ok || pdrx->protCondition == pc_error_crc || pdrx->protCondition == pc_error_cmd)
			{
			if(f.srcID == 70)
				asm("nop");
#ifdef HPROT_ENABLE_RX_RAW_BUFFER
			printRx(pdrx->rxRawBuffer, pdrx->rxRawBufferCount, pdrx->protCondition);
			clearRawBuffer();
#endif
			if(pdrx->protCondition == pc_error_crc)
				{
				...
				if(pdrx->flags.isCrypto)
					{
					int nn=hprotBuffDecrypt(pdrx,rxBuffer,initial_ndata);
			...
			if(pdrx->protCondition == pc_error_cmd)
				{
		...
		if(pdrx->protCondition == pc_ok)
			...
			switch(status)
				...
				case st_ansPending:
					 ...
					 */
					if(f.hdr == HPROT_HDR_ANSWER && f.srcID == ansPendingFrom && f.num == ansFrameNumb)
						{
 ...
 * @param p implementation data
 */
void CommTh::sendCommand(hprot_idtype_t dstId, uint8_t cmd, uint8_t* data, uint8_t len, void *p)
{
pthread_mutex_lock(&mutexTx);
pdtx = impl_getProtocolData('t', dstId);
int n = hprotFrameBuild(pdtx, txBuffer, HPROT_HDR_COMMAND, pdtx->myID, dstId, cmd, data, len, HPROT_SET_FNUMBER_DEFAULT);
 ...
 * @param p implementation data
 */
void CommTh::sendAnswer(hprot_idtype_t dstId, uint8_t cmd, uint8_t* data, uint8_t len, int fnumb, void *p)
{
pthread_mutex_lock(&mutexTx);
pdtx = impl_getProtocolData('t', dstId);
int n = hprotFrameBuild(pdtx, txBuffer, HPROT_HDR_ANSWER, pdtx->myID, dstId, cmd, data, len, fnumb);
pthread_mutex_unlock(&mutexTx);
printTx(txBuffer, n);
impl_Write(txBuffer, n, dstId, p);
}
 ...
 * @param p implementation data
 */
void CommTh::sendRequest(hprot_idtype_t dstId, uint8_t cmd, uint8_t* data, uint8_t len, bool setAnsPending, void *p)
{
pthread_mutex_lock(&mutexTx);
pdtx = impl_getProtocolData('t', dstId);
bool cstatus = pdtx->flags.isCrypto;
pdtx->flags.isCrypto = false;  // temporary disable to have the frame in plain

uint8_t tmp_fn = pdtx->frameNumber;  // save the current frame number
int n = hprotFrameBuild(pdtx, txBuffer, HPROT_HDR_REQUEST, pdtx->myID, dstId, cmd, data, len, HPROT_SET_FNUMBER_DEFAULT);

hprotReset(pdtx);
pdtx->frameNumber = tmp_fn;  // restore the current frame number after pd reset

to.startTimeout(timeout);
if(setAnsPending)
	{
	setCommAnsPending(dstId, pdtx->frameNumber, HPROT_CMD_NONE);
	rxResult.setWaiting();
	}

pdtx->flags.isCrypto = cstatus;
if(pdtx->flags.isCrypto)
	{
	n = hprotBuffCrypt(pdtx, txBuffer, n);
	}
pthread_mutex_unlock(&mutexTx);
printTx(txBuffer, n);
impl_Write(txBuffer, n, dstId, p);
 ...
 * @param p implementation data
 */
void CommTh::sendFrame(uint8_t hdr, hprot_idtype_t srcId, hprot_idtype_t dstId, uint8_t cmd, uint8_t* data, uint8_t len, void *p)
{
pthread_mutex_lock(&mutexTx);
pdtx = impl_getProtocolData('t', dstId);
int n = hprotFrameBuild(pdtx, txBuffer, hdr, srcId, dstId, cmd, data, len, HPROT_SET_FNUMBER_DEFAULT);
pthread_mutex_unlock(&mutexTx);
printTx(txBuffer, n);
impl_Write(txBuffer, n, dstId, p);
}
...
{
bool ret = false;
if(pthread_mutex_trylock(&mutexRx) == 0)
	{
 ...
 */
void CommTh::setCommAnsPending(hprot_idtype_t ansFromID, uint8_t frameNumber, uint8_t ansCmd)
{
pthread_mutex_lock(&mutexRx);
