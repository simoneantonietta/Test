/*
   06-11-2009 - Il polling con NAK impostato ma senza evento in coda viene
                gestito come un polling normale.
*/
...
static void feal_start(CEI_data *cei, int idx, int mode, unsigned char *key, int iter)
{
  ...
  n = iter*2 + 16;
  memset(cei->key[idx].ki, 0, iter*2+16);
...
static int cei_L2_send_packet(ProtDevice *dev, unsigned char *pkt, int len)
{
  ...
  if((dev->consumer == config.debug) && (CEI->L2out[1] > 0))
  {
    int ret;
    buf = malloc(10+CEI->L2out_len*2);
    if(buf)
    {
      sprintf(buf, "CEI TX: ");
      for(ret=0; ret<CEI->L2out_len; ret++)
        sprintf(buf+8+ret*2, "%02x", CEI->L2out[ret]);
      support_log(buf);
printf("%d - %s\n", time(NULL), buf);
      free(buf);
    }
...
void cei_set_timeout(ProtDevice *dev, int timeout)
{
  dev->timeout = timeout;
}
...
static int cei_L2_get_packet_SM(ProtDevice *dev)
{
  ...
  while(1)
  {
    if((state != 3) && (state != 5))
    {
      ...
      {
        if(!dev->timeout) dev->timeout = CEI_TIMEOUT;
        
        to.tv_sec = dev->timeout/10;
        to.tv_usec = (dev->timeout%10)*100000;
        FD_SET(dev->fd, &fds);
...
if(!idx) printf("%d - rx:", time(NULL));
printf("%02x", CEI->L2[idx]);
          //if(didx < 1023) dbuf[didx++] = CEI->L2[idx];
        }
        else if(n == 0)	// timeout
          state = 5;
        else
      ...
      else
      {
        n = read(dev->fd, CEI->L2 + idx, 1);
        if(n <= 0)
          state = 5;	// timeout or error
        //else if(didx < 1023)
        //  dbuf[didx++] = CEI->L2[idx];
      }
    }
    ...
    switch(state)
    {
      ...
      case 3:
...
#if 0
        if((dev->consumer == config.debug) && didx)
        {
          char *buf;
          buf = malloc(10+didx*2);
          if(buf)
          {
            sprintf(buf, "RAW rx: ");
            for(n=0; n<didx; n++)
              sprintf(buf+8+n*2, "%02x", dbuf[n]);
            support_log(buf);
            free(buf);
          }
        }
        ...
        if(!CEI->L4_len)
        {
          /* Verifico se c'è altro in coda, potrei avere più polling accodati
             ad esempio per lantenze GPRS e devo gestire solo l'ultimo. */
          to.tv_sec = 0;
          to.tv_usec = 10000;
          FD_SET(dev->fd, &fds);
          n = select(dev->fd+1, &fds, NULL, NULL, &to);
...
if(idx) printf("\n");
      ...
      case 5:
        ...
        didx = 0;
#endif
        if((dev->consumer == config.debug) && idx)
        {
          char *buf;
          buf = malloc(10+idx*2);
          if(buf)
          {
            sprintf(buf, "CEI Er: ");
            for(n=0; n<idx; n++)
              sprintf(buf+8+n*2, "%02x", CEI->L2[n]);
            support_log(buf);
printf("%d - %s\n", time(NULL), buf);
            free(buf);
          }
...
static int cei_L2_get_packet(ProtDevice *dev)
{
  ...
  if((dev->consumer == config.debug) && (CEI->L2[1] > 0))
  {
    char *buf;
    buf = malloc(10+((CEI->L4-CEI->L2)+CEI->L4_len+3)*2);
    if(buf)
    {
      sprintf(buf, "CEI RX: ");
      for(ret=0; ret<((CEI->L4-CEI->L2)+CEI->L4_len+3); ret++)
        sprintf(buf+8+ret*2, "%02x", CEI->L2[ret]);
      support_log(buf);
printf("%d - %s\n", time(NULL), buf);
      free(buf);
    }
      ...
      }
{
  int ret;
    ...
    /* Il polling con NAK ma senza eventi in coda
       viene considerato un polling normale. */
  }
...
printf("**** -> invio cifrato (len %d)\n", len-8);
            for(ret=0; ret<8; ret+=2) init[ret] = CEI->S;
            for(ret=1; ret<8; ret+=2) init[ret] = CEI->R;
            ret = cei_mac(CEI, pkt+8, FEAL_SK, init, len-8);
            memcpy(pkt+len, &ret, 4);
...
//          pkt[4] |= CEI_L4_FLAG_S;
    ...
    else
    {
      ...
      {
        pkt[4] = (CEI->option << 5)|CEI_L4_FLAG_TA;
      }
    }
  }
...
static int cei_L4_get_packet(ProtDevice *dev)
{
  ...
  if((ret = cei_L3_get_packet(dev)) <= 0)
  {
    ...
    if((ret == -2) && (CEI->flag & CEI_WAITREPLY))
    {
      if(CEI->resend < CEI_MAX_RESEND)
        ret = -3;
      else
      {
        /* Massimo numero di ritrasmissioni, chiudo la connessione */
        CEI->resend = 0;
        ret = -4;
      }
    }
  ...
  if((dev->consumer == config.debug) && (CEI->L4_len > 0))
  {
    char *buf;
    buf = malloc(48);
    if(buf)
    {
      sprintf(buf, "CEI L4: len:%d R:%d/%d S:%d/%d W:%c O:%d",
        CEI->L4_len, CEI->R, CEI->L4[6], CEI->S, CEI->L4[7],
        (CEI->flag & CEI_WAITREPLY)?'1':'0', CEI->outoforder);
      support_log(buf);
printf("%d - %s\n", time(NULL), buf);
      free(buf);
    }
...
printf("*** contatori ok ***\n");
    ...
    /* Per Centrax (MPS) i pacchetti di trasporto sono sempre in chiaro */
    if((CEI->L4[4] & CEI_L4_FLAG_LP) && !(CEI->L4[4] & CEI_L4_FLAG_S) && !(CEI->L4[4] & CEI_L4_FLAG_TA))
    {
...
printf("*** trasporto %d ***\n", ((CEI_trasporto*)(CEI->L4))->funzione);
        ...
        case 255:
          switch(((CEI_trasporto*)(CEI->L4))->dati[1])
          {
            ...
            case 2:
              ...
              /* (14-10-2010) Perché c'era questo break? Devo comunque rispondere! */
              //break;
              break;
            case 3:
            case 4:
  ...
  if((dev->consumer == config.debug) && CEI->L4_len)
  {
    char *buf;
    buf = malloc(10+CEI->L4_len*2);
    if(buf)
    {
      sprintf(buf, "CEI L4: ");
      for(ret=0; ret<CEI->L4_len; ret++)
        sprintf(buf+8+ret*2, "%02x", CEI->L4[ret]);
      support_log(buf);
printf("%d - %s\n", time(NULL), buf);
      free(buf);
    }
