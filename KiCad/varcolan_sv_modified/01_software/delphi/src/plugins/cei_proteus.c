...
static int cei_callback_gprs(int fd, void *null)
{
  ...
  /* Allungo il timeout CEI a 10s per gestire le latenze GPRS */
  cei_set_timeout(dev_main, 100);
  dev_main->fd = fd;
  ((CEI_data*)(dev_main->prot_data))->resend =
    ((CEI_data*)(dev_main->prot_data))->flag = 0;
  ((CEI_data*)(dev_main->prot_data))->S =
    ((CEI_data*)(dev_main->prot_data))->R =
    ((CEI_data*)(dev_main->prot_data))->outoforder = 0;
  pthread_mutex_unlock(&cei_backupmutex);
  
  while(cei_callback_connect) sleep(1);
  
  pthread_mutex_lock(&cei_backupmutex);
  /* Ripristino il timeout CEI al default per la connessione LAN */
  cei_set_timeout(dev_main, CEI_TIMEOUT);
  dev_main->fd = -1;
...
//  PrintEventEx2[Ex2_Delphi][dev.consumer][Ex2_Allineamento] = 0;
  ...
  server_addr = (char*)strdup(config.consumer[dev_main->consumer].param);
...
//printf("*** verifica evento (cons:%d abil:%02x)\n", dev.consumer, LS_ABILITATA[dev.consumer]);
      ...
      if(squillo || cei_recall || ev.Len)
      {
        if(cei_backup_retry)
        {
          ...
          if(!cei_backup_retry)
          {
            sprintf(buf, "CEI (%d) Backup last retry", dev_main->consumer);
            support_log(buf);
            /* Consuma tutti gli ulteriori eventi per evitare che
               si reinneschi l'attivazione. Se non c'è riuscito finora
               è inutile riprovare per ogni singolo evento in coda. */
            /* E intanto azzera ev.Len. */
            while(!codec_get_event(&ev, &dev));
          }
        }
...
#warning DEBUG solo GPRS
        ...
        support_log(log);
        CEI->resend = CEI->flag = 0;
        CEI->S = CEI->R = CEI->outoforder = 0;
      }
      if(backup) pthread_mutex_unlock(&cei_backupmutex);
    }
...
//printf("**** Uscita\n");
      ...
      backup_poll = 0;
      cei_backup_retry = 0;
      sprintf(log, "CEI (%d) Backup normal exit", dev->consumer);
      support_log(log);
    }
    else
    {
    ...
    if(ret == -3)
    {
      sprintf(log, "CEI (%d) Resend packet", dev->consumer);
      support_log(log);
...
#else
static struct _cid {
  unsigned char CIDev[CIDDIM][16];
  unsigned char CIDlastev[16];
  int CIDin;
  int CIDout;
  int CIDseq;
} *CID = NULL;
...
/*
   CID_sending indica come sta avvenendo l'invio
   0: nessun invio in corso
   1: LAN
   2: GPRS primo server
   3: GPRS secondo server
   4: l'evento deve essere inoltrato via backup SMS
   5: in attesa di conferma invio sms
   6: l'evento non è stato spedito, reinnesco della catena dei vettori
*/
...
#define CID_CH_SMS	2
#define CID_CH_CBK_GPRS	3
#define CID_CH_CBK_LAN	4
...
int CID_AS_message(int cond, int num, int evento, int area, int pointid)
{
  ...
  if(((CID->CIDin+1)%CIDDIM) != CID->CIDout)
  {
    memcpy(&(CID->CIDev[CID->CIDin]), buf, 16);
    CID->CIDin++;
    if(CID->CIDin >= CIDDIM) CID->CIDin = 0;
  }
...
void CID_create_ev(void)
{
  ...
  memcpy(CID->CIDlastev, MACaddr, 6);
  CID_strtobin(CID->CIDev[CID->CIDout]);
  CID->CIDlastev[14] = CID->CIDseq++;
  
  c = cei_CRC16(CID->CIDlastev, 15);
  CID->CIDlastev[15] = (c>>8)+c;
  
  CID->CIDout++;
  if(CID->CIDout >= CIDDIM) CID->CIDout = 0;
}
...
#if 1
{
  ...
  s = strlen(m);
  for(i=0; i<16; i++) sprintf(m+s+i*2, "%02x", CID->CIDlastev[i]);
  support_log(m);
}
#endif
  
  if((CID->CIDlastev[9] == 0x16) && (CID->CIDlastev[10] == 0xa2))
  {
    if(CID->CIDlastev[11] != 0xa3) // non è callback gprs
    {
      if(CID->CIDlastev[11] == 0xa4)	// callback LAN
        CID->CIDlastev[11] = 0xa3;	// callback gprs
      else
        CID->CIDlastev[11] = 0xa1;	// stato in vita gprs
      /* Ricalcolo cks ContactID e crc messaggio */
      for(i=0; i<8; i++)
        sprintf(rec+i*2, "%02X", CID->CIDlastev[i+6]);
      rec[15] = 0;
      for(i=0; i<15; i++)
      {
        if(rec[i] == 'A')
      ...
      i = cei_CRC16(CID->CIDlastev, 15);
      CID->CIDlastev[15] = (i>>8)+i;
    }
  }
  ...
  while(connto)
  {
    if(!CID->CIDlastev[8] && (CID->CIDout != CID->CIDin) /*CIDev[CID->CIDout][4]*/)
    {
      CID_create_ev();
...
#if 1
{
  ...
  support_log(m);
      ...
      if((CID->CIDlastev[9] == 0x16) && (CID->CIDlastev[10] == 0xa2))
      {
        if(CID->CIDlastev[11] != 0xa3) // non è callback
        {
          CID->CIDlastev[11] = 0xa1;	// gprs
          /* Ricalcolo cks ContactID e crc messaggio */
          for(i=0; i<8; i++)
            sprintf(rec+i*2, "%02X", CID->CIDlastev[i+6]);
          rec[15] = 0;
    ...
    if(!CID_send_to && CID->CIDlastev[8])
    {
      if(CID_send_retry < 2)
      {
        support_log("ContactID (AS): gprs invio");
        write(fd, CID->CIDlastev, 16);
        //if((CID->CIDlastev[9] != 0x16) || (CID->CIDlastev[10] != 0xa2))
        {
      ...
      else if((CID->CIDlastev[9] == 0x16) && (CID->CIDlastev[10] == 0xa2))
      {
...
void CID_AS_sms(int esito, void *data)
{
  if(esito == 0)
  {
    /* Invio sms regolare */
  ...
  {
    /* L'sms non è stato inviato, riparto con il vettore base */
    support_log("ContactID (AS): Invio sms fallito");
    CID_sending = 6;
  }
}
...
printf("\n");
  ...
  if(!CID->CIDlastev[8] && (data[9] == 0x16) && (data[10] == 0xa2))
  {
    /* E' una conferma di stato in vita ricevuta in modo inatteso. La scarto. */
  }
  else if(!memcmp(data, CID->CIDlastev, 16))
  {
  ...
  else
  {
    ...
    switch(crc)
    {
      case 0x86a2:	// impostazione periodo stato in vita
        ...
        if(data[11] == 0xa1)
        {
          /* Forzatura backup su GPRS */
          CID_AS_message(1, CIDbackup, 1602, CID_CH_CBK_GPRS, CIDkeepalivebase/600);
          if(ch == CID_CH_LAN)
          {
            CID_sending = 2;	// backup gprs
            /* Se non ho già un evento in corso di invio, preparo quello appena accodato. */
            if(!CID->CIDlastev[8]) CID_create_ev();
...
static void cid_loop(ProtDevice *dev)
{
  ...
  if(Restart) unlink("/tmp/cidas.events");
  fd[1] = open("/tmp/cidas.events", O_RDWR|O_CREAT/*|O_EXCL|O_TRUNC*/, 0600);
  if(fd[1] >= 0)
  {
    ftruncate(fd[1], sizeof(struct _cid));
    CID = mmap(NULL, sizeof(struct _cid), PROT_READ|PROT_WRITE, MAP_SHARED, fd[1], 0);
    close(fd[1]);
  }
  if(!CID) CID = calloc(1, sizeof(struct _cid));
  
  /* Se al riavvio trovo un evento preparato per l'invio,
     riattivo la spedizione per non perderlo. */
  if(CID->CIDlastev[8]) CID_sending = 6;
  ...
  while(1)
  {
    ...
    if(!n)
    {
      ...
      if((CID_sending == 1) && CID_send_to)
      {
        ...
        if(!CID_send_to)
        {
          if(CID_send_retry < 2)
          {
            CID_send_retry++;
            CID_send_to = 20;
            send(fd[0], CID->CIDlastev, 16, 0);
          }
          else if(CID_server_addr[1] && (CID_send_retry < 6))
          {
            CID_send_retry++;
            CID_send_to = 20;
            send(fd[1], CID->CIDlastev, 16, 0);
          }
          else
          {
      ...
      else if(CID_sending == 4)
      {
        ...
        /* Se non è già stato codificato (es: manca LAN e GPRS non si connette)
           lo devo codificare ora per poter avanzare il contatore CID->CIDout. */
        if(!CID->CIDlastev[8] && (CID->CIDout != CID->CIDin) /*CIDev[CID->CIDout][4]*/)
        {
          CID_create_ev();
        }
...
#if 0
        if((CIDbackup > 0) && (CID->CIDlastev[9] != 0x16) && (CID->CIDlastev[10] != 0xa2) &&
           /* Sono capitati degli invii di sms con testo nullo */
           (CID->CIDlastev[9] != 0) && (CID->CIDlastev[10] != 0) && (CID_SMS_limit < CID_SMS_LIMIT))
#else
        if((CIDbackup > 0) && !((CID->CIDlastev[9] == 0x16) && (CID->CIDlastev[10] == 0xa2)) &&
           /* Sono capitati degli invii di sms con testo nullo */
           !((CID->CIDlastev[9] == 0) && (CID->CIDlastev[10] == 0)) && (CID_SMS_limit < CID_SMS_LIMIT))
#endif
        {
          ...
          CID_sending = 5;
          /* Imposto anche un timeout ragionevolmente lungo, per cui se per qualsiasi
             ragione l'invio dell'sms non dovesse essere gestito, prima o poi esco
             comunque dall'attesa dell'esito.
             Male che vada l'evento verrà poi inviato due volte. */
          /* Timeout 5 minuti */
          CID_send_to = 3000;
          
          for(n=0; n<16; n++)
            sprintf(buf+n*2, "%02X", CID->CIDlastev[n]);
          if(GSM_SMS_send_callback(CIDbackup, buf, CID_AS_sms, NULL) < 0)
          {
            /* Errore invio (es: rubrica non definita) */
            support_log("ContactID (AS): backup sms ** errore");
            CID_sending = 6;
          }
          else
            CID_SMS_limit++;
        ...
        {
          if((CID->CIDlastev[9] == 0) && (CID->CIDlastev[10] == 0))
            support_log("ContactID (AS): backup sms NULLO!");
          else if(CID_SMS_limit >= CID_SMS_LIMIT)
            support_log("ContactID (AS): limite SMS");
          /* Se è un evento di allarme e non è definito il backup SMS
             oppure si è raggiunto il limite di invio SMS, reinnesco
             l'invio dal vettore base anziché buttare via l'evento */
          if(!((CID->CIDlastev[9] == 0x16) && (CID->CIDlastev[10] == 0xa2)) &&
             !((CID->CIDlastev[9] == 0) && (CID->CIDlastev[10] == 0)) &&
             (!(CIDbackup > 0) || !(CID_SMS_limit < CID_SMS_LIMIT)))
          {
            CID_sending = 6;
          }
          else
      ...
      else if((CID_sending == 5) && CID_send_to)
      {
        /* Ho un evento in attesa di essere inviato tramite backup SMS.
           Se l'esito non ritorna in tempi ragionevoli allora sblocco
           e reinnesco il vettore base per l'invio. L'evento non viene
           scartato. */
        CID_send_to--;
        if(!CID_send_to) CID_sending = 6;
      }
      
      if(CIDkeepalive) CIDkeepalive--;
      /* Non invio un nuovo stato in vita se sto già tentanto di inviarne uno. */
      if(!CIDkeepalive && ((CID->CIDlastev[9] != 0x16) || (CID->CIDlastev[10] != 0xa2)))
      {
    ...
    if(GSM_Squillo_sconosciuto())
    {
      ...
      /* Non più, lo tato in vita va inviato sul vettore attivo, la forzatura
         su GPRS avviene tramite comando specifico. */
...
#else
      /* Accodo l'evento di callback LAN, se poi viene trasmesso via GPRS viene
         cambiato automaticamente il vettore da callback LAN a callback GPRS. */
      CID_AS_message(1, CIDbackup, 1602, CID_CH_CBK_LAN, CIDkeepalivebase/600);
#endif
    }
    
    pthread_mutex_lock(&CIDmux);
    if((!CID_sending && (CID->CIDout != CID->CIDin)) ||
       (CID_sending == 6))
    {
      /* Se non ho ancora l'evento fallito via sms, creo il nuovo evento */
      if(!CID_sending) CID_create_ev();
