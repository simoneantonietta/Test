...
#ifdef DEBUG
printf("\nIN: Func: %02x\n", req->pdu.func);
for(i=0; i<len; i++) printf("%02x ", ((unsigned char*)req)[i]);
printf("\n");
#endif
...
#ifdef DEBUG
printf("OUT: Func: %02x\n", resp.pdu.func);
for(i=0; i<(ldata+4); i++) printf("%02x ", ((unsigned char*)&(resp.ap.ui))[i]);
printf("\n");
#endif
...
#ifdef DEBUG
printf("OUT: Func: %02x\n", resp.pdu.func);
for(i=0; i<(ldata+2+sizeof(MBAP)-1); i++) printf("%02x ", ((unsigned char*)&resp)[i]);
printf("\n");
#endif
...
printf("reqidx %d (%d)\n", reqidx[idx], (bswap_16(*(unsigned short*)(req[idx]+4))+6));
