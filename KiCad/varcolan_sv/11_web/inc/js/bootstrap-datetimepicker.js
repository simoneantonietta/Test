  ...
  DateTimePicker.prototype = {
    ...
    init: function(element, options) {
      ...
      if (this.pickTime) {
        ...
        if (!this.timeIcon) this.timeIcon = 'time';
        icon.addClass(this.timeIcon);
      }
      if (this.pickDate) {
        if (icon && icon.length) this.dateIcon = icon.data('date-icon');
        if (!this.dateIcon) this.dateIcon = 'calendar';
        icon.removeClass(this.timeIcon);
        icon.addClass(this.dateIcon);
      }
	  if(element.closest('.modal-body')) {
//	  if($('.modal').length) {
	      this.widget = $(getTemplate(this.timeIcon, options.pickDate, options.pickTime, options.pick12HourFormat, options.pickSeconds, options.collapse)).appendTo('.modal');
	  }
	  else {
...
//      this.height = !this.isInput ? this.component.outerHeight() : this.$element.outerHeight();
      this.place();
...
//	  var offset = !this.isInput ? this.component.offset() : this.$element.offset();
      this.width = this.component ? this.component.outerWidth() : this.$element.outerWidth();
//      this.width = !this.isInput ? this.component.outerWidth() : this.$element.outerWidth();
      offset.top = offset.top + this.height;

	  if( this.$element.closest('.modal-body') ){
		offset.top = offset.top + $('.modal').scrollTop();
      }
        ...
        '<div class="bootstrap-datetimepicker-widget dropdown-menu">' +
          '<ul>' +
            '<li' + (collapse ? ' class="collapse in"' : '') + '>' +
              ...
              '</div>' +
            '</li>' +
            '<li class="picker-switch accordion-toggle"><a><span class="glyphicon glyphicon-' + timeIcon + '"></span></a></li>' +
